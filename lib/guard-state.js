...
export class LoopGuardState {
  ...
  beginTurn(agent, turn, stopRequested) {
    ...
    if (prior === undefined || prior.turn !== turn) {
      this.byAgent.set(key, { turn, attempts: 0, repeatGroups: new Map(), fingerprints: new Set(), stopRequested: Boolean(stopRequested) });
      return;
    }
  ...
  denyReason(agent, name, args) {
    ...
    if (state === undefined) {
      state = { turn: -1, attempts: 0, repeatGroups: new Map(), fingerprints: new Set(), stopRequested: false };
