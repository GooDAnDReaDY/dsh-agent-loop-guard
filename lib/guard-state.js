...
export class LoopGuardState {
  constructor(config) {
    ...
    this.config = {
      ...config,
      maxToolAttemptsPerTurn: positiveInteger(config.maxToolAttemptsPerTurn, 8),
  ...
  denyReason(agent, name, args) {
    ...
    const callsInRepeatGroup = (state.repeatGroups.get(repeatFingerprint) ?? 0) + 1;
    if (callsInRepeatGroup > this.config.maxCallsPerRepeatGroup) return 'LOOP_GUARD_REPEAT: near-identical tool calls were blocked; stop the loop and answer with available evidence.';
    if (this.progressToolNames.has(String(name))) {
      if (state.progressAttempts >= this.config.maxProgressToolCallsPerTurn) {
      ...
      if (state.attempts >= this.config.maxToolAttemptsPerTurn) {
    ...
    state.repeatGroups.set(repeatFingerprint, callsInRepeatGroup);
    return undefined;
  }
}
