...
- **Документация:**
  ...
  - `docs/README.ru.md` (Русский, синхронизированная проектная документация)
  - `docs/README.zh.md` (中文, обязательная пользовательская документация)
  - `AGENTS.md` (правила архитектуры и зафиксированные решения)
...
- **Утверждённые референсы:** Нативные компоненты DSH Web UI (`settingsScope`, css-переменные темы `--dsw-alias-*`).
- **Не копировать:** Кастомные несогласованные темы, яркие несистемные градиенты, сторонние тяжелые библиотеки компонентов (MUI, AntD).
...
- **Цвета и роли:**
  ...
  - Третичный текст / стрелки: `var(--dsw-alias-label-tertiary)`
  - Акцент обновления / бейдж: `var(--dsw-alias-accent-primary, #3b82f6)`
  - Предупреждения / ошибки: `var(--dsw-alias-status-danger, #ef4444)`
  - Успешное сохранение / актуальность: `var(--dsw-alias-status-success, #22c55e)`
- **Типографика:**
...
- **Accessibility:**
  - Семантические теги `<label>`, ассоциированные с input.
...
- **Компоненты:**
  1. `PluginCard`: Сворачиваемый контейнер карточки настроек.
...
- **Do:**
  ...
  - Документация в репозитории ведётся на 3 языках (`README.md`, `docs/README.ru.md`, `docs/README.zh.md`).
- **Don't:**
